...
const ShiftSwapManager = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:7070/api/swapshifts')
      .then(response => {
        const sortedData = response.data.sort((a, b) => (a.isAccepted === null) ? -1 : ((b.isAccepted === null) ? 1 : 0));
  ...
  const handleSwapApproval = (index, isApproved) => {
    ...
    setShiftSwaps(updatedSwaps.sort((a, b) => (a.isAccepted === null) ? -1 : ((b.isAccepted === null) ? 1 : 0)));
  };
          ...
          {shiftSwaps.map((swap, index) => (
            <tr key={index}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${swap.isAccepted ? (swap.isAccepted === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') : 'bg-gray-100 text-gray-500'}`}>
                  {swap.isAccepted || 'Pending'}
                </span>
              </td>
