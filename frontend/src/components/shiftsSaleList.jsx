...
const ShiftsSaleList = ({ user }) => {
    ...
    const approveShift = (shift) => {
        const confirm = window.confirm("Are you sure you want to approve shift with id " + shift.id + " ?")
        if (confirm) {
    ...
    const declineShift = (shift) => {
        const confirm = window.confirm("Are you sure you want to decline shift with id" + shift.id + " ?")
        if (confirm) {
    ...
    const bidOnShift = (shift) => {
        const confirm = window.confirm("Are you sure you want to bid on this shift?")
        if (confirm) {
                        ...
                        shifts.map((shift) => (
                            <Card key={shift.id} className="mb-4 ml-1.5">
                                <CardHeader>
                                    ...
                                    <CardTitle className={"mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white"}><span className="font-bold">Seller:</span> {shift.username}</CardTitle>
                                </CardHeader>
                                <CardContent className="mt-5">
                                    <CardDescription><span className="font-bold">Shift start:</span> {new Date(Date.UTC(...shift.shiftStart)).toLocaleDateString()} {new Date(Date.UTC(...shift.shiftStart)).toLocaleTimeString()}</CardDescription>
                                    <CardDescription><span className="font-bold">Shift ends:</span> {new Date(Date.UTC(...shift.shiftEnd)).toLocaleDateString()} {new Date(Date.UTC(...shift.shiftEnd)).toLocaleTimeString()}</CardDescription>
