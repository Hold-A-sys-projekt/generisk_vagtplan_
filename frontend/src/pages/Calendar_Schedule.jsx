...
function Calendar_Schedule() {
  ...
  const handleSelect = ({ start, end }) => {
    ...
    let startTime = window.prompt(
      "Enter start time for the event (HH:mm)",
      "00:00"
    );
    let endTime = window.prompt(
      "Enter end time for the event (HH:mm)",
      "00:00"
    );
    ...
    if (startTime && endTime) {
      ...
      const shiftType = window.prompt(
        "Enter shift type (Covered, Pending, For Sale):"
      );
      const shiftColors = {
        Covered: "#4CAF50",
        Pending: "#FFC107",
      ...
      const newEvent = {
        id: events.length + 1,
        title: `${title} - ${shiftType} | ${format(
          startDateTime,
          "HH:mm"
        )} - ${format(endDateTime, "HH:mm")} (${duration})`,
        start: startDateTime,
        end: endDateTime,
        allDay: false,
        type: shiftType,
  ...
  const handleSelectEvent = (event) => {
    const editChoice = window.prompt(
      "Do you want to edit or delete the event? (edit/delete)",
      "edit"
    );
    if (editChoice.toLowerCase() === "delete") {
      ...
      const newType = window.prompt(
        "Edit shift type (Covered, Pending, For Sale):",
        event.type
      );

      if (newStartTime && newEndTime && newType) {
        let startDateTime = parseISO(
          `${format(event.start, "yyyy-MM-dd")}T${newStartTime}`
        );
        let endDateTime = parseISO(
          `${format(event.end, "yyyy-MM-dd")}T${newEndTime}`
        );
