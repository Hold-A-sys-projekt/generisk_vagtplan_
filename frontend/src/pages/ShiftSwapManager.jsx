...
// Format date from array [year, month, day, hour, minute]
...
const ShiftSwapManager = () => {
        ...
        axios.get('http://localhost:7070/api/swapshifts')
            .then(response => {
                ...
                setShiftSwaps(sortedData.map(swap => ({
                    id: swap.id,
                    shift1Day: formatDate(swap.shift1Start),
                    shift1User: swap.shift1User,
                    shift1Time: `${formatTime(swap.shift1Start)} - ${formatTime(swap.shift1End)}`,
                    shift2Day: formatDate(swap.shift2Start),
                    shift2User: swap.shift2User,
                    shift2Time: `${formatTime(swap.shift2Start)} - ${formatTime(swap.shift2End)}`,
    ...
    const handleSwapApproval = (index, isApproved) => {
        axios.post(`http://localhost:7070/api/swapshifts/${shiftSwaps[index].id}/approve`, null, { params: { accepted: isApproved } })
            .then(() => {
                ...
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${swap.isAccepted === 'Approved' ? 'bg-green-100 text-green-800' : (swap.isAccepted === 'Not Approved' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-500')}`}>
                  {swap.isAccepted}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button onClick={() => handleSwapApproval(index, 'Approved')} disabled={swap.isAccepted !== 'Pending'} className={`text-white bg-green-500 hover:bg-green-600 font-medium rounded-lg text-sm px-3 py-1 mr-2 ${swap.isAccepted !== 'Pending' ? 'opacity-50 cursor-not-allowed' : ''}`}>Approve</button>
                            <button onClick={() => handleSwapApproval(index, 'Not Approved')} disabled={swap.isAccepted !== 'Pending'} className={`text-white bg-red-500 hover:bg-red-600 font-medium rounded-lg text-sm px-3 py-1 ${swap.isAccepted !== 'Pending' ? 'opacity-50 cursor-not-allowed' : ''}`}>Decline</button>
                        </td>
                    </tr>
                ))}
                </tbody>
